...
def render_private_plan():
    st.markdown("### 🛡️ 私家無憂組 申請表")
    # 收集必要欄位
    owner = st.text_input("👤 主人姓名")
    pet_name = st.text_input("🐾 寶貝名稱")
    chipped = st.text_input("🔖 晶片號碼 (NA)")
    phone = st.text_input("📞 聯絡電話")
    email = st.text_input("✉️ 電郵地址")
    weight_input = st.text_input("⚖️ 體重 (kg)")
    deductible_key = st.selectbox("自付方案", ["請選…","🍀 綠意款","🌸 櫻花款","🎉 戰鼓款"])
    reimbursement_key = st.selectbox("理賠方案", ["請選…","🌻 90%","❄️ 80%","🍁 70%"])
    term = st.selectbox("繳費期數", [3,6,12], format_func=lambda x: f"{x} 個月")

    if st.button("💌 送出申請"):
        # 驗證
        errors = []
        if not owner: errors.append("主人姓名")
        if not pet_name: errors.append("寶貝名稱")
        if not phone: errors.append("電話")
        if not email or '@' not in email: errors.append("有效電郵")
        try:
            weight = float(weight_input)
        except:
            errors.append("正確體重")
        ded_map = {"🍀":0.1,"🌸":0.15,"🎉":0.2}
        rr_map = {"🌻":0.9,"❄️":0.8,"🍁":0.7}
        dr = ded_map.get(deductible_key[:2])
        rr = rr_map.get(reimbursement_key[:2])
        if dr is None: errors.append("自付方案")
        if rr is None: errors.append("理賠方案")
        if errors:
            st.error("請補充：" + ",".join(errors))
            return

        # 計算
        total, extra, total_extra = premium_calculation_private(
            weight=weight,
            age=2,  # 範例，請帶入真實年齡
            term=term,
            deductible_rate=dr,
            reimbursement_rate=rr,
        ...
        st.write(f"**總金額**: MOP {total:.2f}")
        st.write(f"**手續費**: MOP {extra:.2f}")

        # 儲存
        record = {
            "owner":owner, "pet_name":pet_name, "chipped":chipped,
            "phone":phone, "email":email,
            "plan_type":"私家無憂組", "covered":"",
            "deductible_rate":dr, "reimbursement_rate":rr,
            "term":term, "monthly_premium":total/term,
            "total_monthly_premium":total,
            "monthly_extra":extra, "total_extra":total_extra,
            "wechat_id":"", "comment":"",
            "cover_consultation":0, "cover_rabies_vax":0,
            "cover_dhppil":0, "cover_corona_vax":0,
            "cover_lyme_vax":0, "cover_bordetella":0
        }
        save_application(record, db_path)
        st.success("✅ 已儲存私家無憂組申請！")
